...
import rameshm.llmeng.llmchat.file_handler_OLD as file_handler

logger = set_environment_logger()

# Enhanced UI with chat management
def create_gr_app(self):
    ...
    with gr.Blocks(title="Multi-LLM Chatbot", theme=gr.themes.Soft()) as multi_model_chat:
        ...
        current_chat_id: Optional[str, None] = gr.State(None)           # Store current chat ID
        chat_list: gr.State[List[Dict[str,LlmChat]]] = gr.State([])     # Store chat list as an array of dictionaries with key as chat_id and value as llm_chat
        chat_history: gr.State[List[str,str]] = gr.State([])            # Store chat history as an array of dictionaries with role and content
